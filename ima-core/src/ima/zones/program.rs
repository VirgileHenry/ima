...
/// Represent a program in the IMA, in visual debug mode.
/// All lines of the program are kept, with the instructions, but also comments and labels.
/// Every line also has a stop flag, which indicates breakpoints.
pub struct VisualDebugModeProgram(pub Vec<(Line, bool)>);
...
impl DebugModeProgram {
    pub fn to_visual_debug(self) -> VisualDebugModeProgram {
        VisualDebugModeProgram(self.0)
    }
}

impl RunMode for VisualDebugModeProgram {
    fn fetch(&self, pc: CodeAddr) -> Option<&Instruction> {
        match self.0.get(pc as usize) {
            Some(line) => line.0.instruction.as_ref(),
            None => None,
...
impl Program<DebugModeProgram> {
    ...
    pub fn to_visual_debug(self) -> Program<VisualDebugModeProgram> {
        Program {
            pc: self.pc,
...
impl Program<VisualDebugModeProgram> {
    /// Toggle the breakpoint of the given line.
    pub fn toggle_breakpoint(&mut self, at: CodeAddr) {
        let mut at = at;
        loop {
