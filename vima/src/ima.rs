...
impl<'a, B: Backend> VisualIMA<'a, B> {
    ...
    fn execute_instr(&mut self) {
        ...
        let result = self.execute(instruction.clone(), &mut input, &mut output).map_err(|e|
            ImaError::ExecutionError{
                error: e,
                line: self.ima.code.pc(),
                instruction
            }
        );
    ...
    fn execute_until_breakpoint(&mut self) {
        loop {
            self.execute_instr();
            ...
            match self.ima.control_flow {
                ImaControlFlow::Continue => (),
                ImaControlFlow::Halt => break,
                ImaControlFlow::Error => break,
            }
        }
    }
}
